...
# model code
cat(file = "full_annual_cycle_model_bernoulli.txt", "
model {
  ...
  ## 6 = early breeding
  ## breeding outcomes: 1 = success, 2 = failure (either failed attemt or deferral)
  ...
  for(i in 1:nind){ # individual index
    alpha_BS_spring[i] ~ dnorm(0, tau_BS_spring)
  } # i
  tau_BS_spring <- pow(sd_BS_spring, -2)
  sd_BS_spring ~ dunif(0, 10)
  ...
  # sub-seasonal betas
  for(s in 1:n_sub_seasons){ # sub-season index
    beta_ODBA_spring[s] ~ dnorm(mu_beta_ODBA_spring, tau_beta_ODBA_spring)
  } # s
  tau_beta_ODBA_spring <- pow(sd_beta_ODBA_spring, -2)
  ...
  for(t in 1:nyears){ # year index
    eps_year_spring[t] ~ dnorm(0, tau_year_spring)
  } # t
  tau_year_spring <- pow(sd_year_spring, -2)
  sd_year_spring ~ dunif(0, 10)
  ...
  ## effects of breeding success, weather, and habitat use on behavior
  # random intercepts for individual
  ...
  ## effects of behavior on breeding success
  for(a in 1:nobs1){ # individual/year index
    BS_spring[a] ~ dbern(p[ids1[a], years1[a]])
    
  # link function
    logit(p[ids1[a], years1[a]]) <- alpha_BS_spring[ids1[a]] + 
      beta_ODBA_spring[1] * mu_log_ODBA_spring[ids1[a], years1[a], 1] +
      beta_ODBA_spring[2] * mu_log_ODBA_spring[ids1[a], years1[a], 2] +
      beta_ODBA_spring[3] * mu_log_ODBA_spring[ids1[a], years1[a], 3] +
...
jags_data <- list(
  ...
  BS_spring = spring_dat_annual$breeding_success,
  ...
  BS_autumn5 = autumn_dat_all$breeding_success2,
  # 6. f loop (autumn model, individual/year/(non-migratory) sub-season index)
